...
@plac.annotations(
    ...
    ensemble=(
        "Ensemble type for sampling, at the moment only 'canonical' is supported.",
        "option",
        "ens",
        str,
    ),
    sublattice_indices=(
        "Indices of sublattices to be used in the sampling, see documentation of WangLandau class.",
        "option",
        "sli",
        list,
    ),
    chemical_potentials=(
        "Chemical potentials for each sublattice, e.g. {'Al': 0.0, 'Si': 0.0}, not used at the moment.",
        "option",
        "cpot",
        dict,
    ),
    predict_swap=(
        "Predict swap moves during sampling, default is True.",
        "option",
        "ps",
        bool,
    ),
    error_reset=(
        "Reset swap prediction after this many iterations, default is None (no reset).",
        "option",
        "errr",
        int,
    ),
    ...
    energy_bin_width=(
        "Energy bin width for sampling.",
        "option",
        "ebw",
        float,
    ),
    f_range=(
        "Range of modification factors for WL-sampling.",
        "option",
        "fr",
        list,
    ),
    update_method=(
        "Method to update the modification factor, default is 'square_root', Options: 'square_root'.",
        "option",
        "um",
        str,
    ),
    flatness_conditions=(
        "Flatness conditions for the histogram, list of lists with flatness and factor pairs.",
        "option",
        "fc",
        list,
    ),
    initial_decoration=(
        "Initial decoration of the supercell, list of integers, defaults to random decoration.",
        "option",
        "id",
        list,
    ),
    filename=(
        "Filename to save the configurational density of states (CDOS) during sampling.",
        "option",
        "fn",
        str,
    ),
    restart_from_file=(
        "Restart sampling and CDOS calculation from a file, located at 'filename'. Default is False.",
        "option",
        "rff",
        bool,
    ),
    plot_hist_real_time=(
        "Plot histogram in real time during sampling, default is True.",
        "option",
        "phrt",
        bool,
    ),
    seed=(
        "Seed for random number generator, default is 1.",
        "option",
        "s",
        int,
    ),
...
def wang_landau(
    ...
    **kwargs,
    ...
    wl.wang_landau_sampling(
        ...
        niter_per_sweep=niter_per_sweep,
        nproc=nproc,
        seed=seed,
        **kwargs,
    )
