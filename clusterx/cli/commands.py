...
modules = [
    ...
    and not f.endswith("main.py")
    and not f.endswith("commands.py")
...
for module in modules:
    for command in getattr(
        __import__("clusterx.cli." + module, fromlist=[module]), "commands"
    ):
        commands.append(command)
        setattr(
            sys.modules[__name__],
            command,
            getattr(__import__("clusterx.cli." + module, fromlist=[module]), command),
        )
