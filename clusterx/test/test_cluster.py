...
cluster_list = [Cluster([0, 1, 2], [13, 14, 15]), Cluster([0], [13])]
...
def test_order_transitive(a, b, c):
    ...
    if b >= a and c >= b:
        assert c >= a, "Transitive property of order relation failed"
...
def test_order_strict_transitive(a, b, c):
    ...
    if b > a and c > b:
        assert c > a, "Transitive property of strict order relation failed"
...
def test_order_antisymmetric(a, b):
    ...
    if b >= a and a >= b:
        assert b == a, "Antisymmetric property of order relation failed"
...
def test_order_strict_asymmetric(a, b):
    ...
    if b > a:
        assert not a > b, "Asymmetric property of strict order relation failed"
...
def test_not_equal():
    ...
    assert c1 != c2, (
        "False equivalence with different number of sites and number of species"
    )
...
def test_exceptions():
    with pytest.raises(
        ValueError,
        match="Initialization error, number of sites in cluster different from number of species.",
    ):
        Cluster([0], [13, 14])
...
def test_inclusion():
    ...
    assert c4 not in clusters, (
        "Non-equivalent cluster should not be found in cluster list"
    )
