...
def grid_mapping(grid: np.ndarray, i_grid: np.ndarray, p_reduced: tuple):
...
def normalize_shape_input(sc_shape):
    ...
    if sc_shape is None:
        ...
        and len(sc_shape) == 3
        and all(isinstance(x, int) for x in sc_shape)
    ):
        ...
        raise ValueError(
            "Invalid sc_shape format: must be None, int, 3-vector of ints, or 3x3 matrix of ints."
        )


def normalize_nsubs_list(
