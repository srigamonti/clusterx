    ...
    **Methods:**
    ...
    def set_radii(self, sdistances, distances, npoints=[], radii=[]):
        ...
        if len(radii) > 0:
            if np.amax(radii) > dmax2:
                sys.exit(
                    "Containing supercell is too small to find clusters pool. Read documentation for ClustersPool class."
                )
        else:
                ...
                for idxs_ in tqdm(
                    ...
                    if _radius <= radius or radius < 0:
                        ...
                        for ss in product(*sites_arrays):
                            ...
                            if tuple(sigmas) not in full_list:
                                cpool_idx_ss.append([idxs, list(ss)])
                                cpool_maxradii.append(radius)

                            for per in symper:
                                sigmas_per = sigmas[np.ix_(per)]
                                full_list.add(tuple(sigmas_per))

        for i, idx_ss in tqdm(
            enumerate(cpool_idx_ss),
            total=len(cpool_idx_ss),
            desc="INFO(clusters_pool): Creating clusters objects",
            disable=disable_tqdm,
        ):
            sps = []
    ...
    def write_clusters_db(
        ...
        **Parameters:**
        ...
        cpool_atoms = self.get_cpool_atoms(
            ...
            vacancy_atomic_number=vacancy_atomic_number,
        )
    ...
    def get_cluster_orbit(
        ...
        **Parameters:**
        ...
        if cluster_sites is not None and cluster_positions is not None:
            print(
                "ERROR (clusterx.clusters.clusters_pool.gen_cluster_orbit): One of cluster_sites or cluster_positions must be None"
            )
    ...
    def get_containing_supercell(self, tight=False):
        ...
        for i in range(3):
            ...
            h[i] = np.dot(cell[i], c[i] / np.linalg.norm(c[i]))
        ...
        if rmax == 0:
            ...
            if tight:
                m = np.diag(
                    [int(n) for n in np.ceil(rmax / h)]
                )  # number of repetitions of unit cell along each lattice vector to contain largest cluster
            else:
                m = np.diag(
                    [int(n) for n in np.ceil(2 * rmax / h)]
                )  # number of repetitions of unit cell along each lattice vector to contain largest cluster
    ...
    def display_info(self, ecis=None):
        ...
        **Parameters**:
        ...
        warnings.warn(
            "The name of this function has changed. Please use `print_info()`.",
    ...
    def print_info(self, ecis=None):
        ...
        **Parameters**:
        ...
        if ecis is None:
            ...
            info_str += "\n|                             Clusters Pool Info                                |"
            info_str += "\n+-------------------------------------------------------------------------------+"
            info_str += "\n|{0:^19s}|{1:^19s}|{2:^19s}|{3:^19s}|".format(
            ...
            info_str += "\n|                                       Clusters Pool Info                                          |"
            info_str += "\n+---------------------------------------------------------------------------------------------------+"
            info_str += "\n|{0:^19s}|{1:^19s}|{2:^19s}|{3:^19s}|{4:^19s}|".format(
            ...
            if option_2 == 2:
                for sp_cl in sps_cl:
                    cluster_sites.append(
                        np.where(np.all(sps_sc == sp_cl, axis=1))[0][0]
                    )  # THIS MAY NEED TO BE REPLACED WITH utils.isclose() !!!
    ...
    def gen_orbit_slow_version(
        ...
        **Parameters:**
        ...
        sp0 = get_scaled_positions(
            p0, self._plat.get_cell(), pbc=super_cell.get_pbc(), wrap=False
        ...
        mult = len(
            orbit0
        )  # Multiplicity of the cluster with respect to the point symmetries of the parent lattice (not the supercell). This is the standard cluster multiplicity used in literature.
