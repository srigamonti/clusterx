...
def scatter_plot(ax, x_data, y_data, marker, color, label):
    ...
    ax.scatter(x_data, y_data, marker=marker, s=15, edgecolors=color, facecolors='none', label=label)
...
def auto_scale_yaxis(property_data, data):
    ...
    ymin = np.amin(property_data)
    ymax = np.amax(property_data)
    for key in ["predicted-property", "predicted-property-cv", "predicted-property-enumeration", "predicted-property-gss"]:
        if data[key] is not None:
            ymin = min(ymin, np.amin(data[key]))
            ymax = max(ymax, np.amax(data[key]))
    dy = ymax - ymin
